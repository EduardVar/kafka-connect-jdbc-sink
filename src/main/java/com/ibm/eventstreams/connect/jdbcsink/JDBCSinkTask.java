 ...
 public class JDBCSinkTask extends SinkTask {
     ...
     public void put(Collection<SinkRecord> records) {
         ...
         try {
             ...
             long startDate = Date.from(start).getTime()/1000;   //unix timestamp
             ...
             // POST data ingest json data to be used by dashboard.
             String jsonString = new JSONObject()
                   .put("startDate", startDate)
                   .put("timeElapsed", timeElapsed)
                   .put("numRecords", records.size())
                   .toString();
 
             logger.info("JSON string being sent: " + jsonString);
 
             CloseableHttpClient httpClient = HttpClientBuilder.create().build();
             try {
                 HttpPost request = new HttpPost("http://localhost:8081/write");
